...
import { Mail, Lock, Eye, EyeOff, User, BookOpen, GraduationCap, Percent, AlertCircle } from 'lucide-react';
import { useAuth } from '@/lib/auth';
import { Input, Button } from '@/components/ui';

export default function Register() {
  const { register, logout, user, loading } = useAuth();
  ...
  const validateForm = () => {
    ...
    if (!formData.cgpa) {
      ...
      if (isNaN(parsedCGPA) || parsedCGPA < 0 || parsedCGPA > 10) {
        tempErrors.cgpa = 'CGPA must be a valid number between 0.0 and 10.0';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        // Clear automatic session to force manual login verification
        logout();
        ...
        <div className="glass-strong rounded-3xl p-8 border border-surface-200/50 dark:border-white/[0.08] shadow-2xl relative">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-surface-500 dark:text-surface-450 mt-1">
              Join Placify to track targets and analyze your profile.
            </p>
          </div>
          ...
          {generalError && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              className="flex items-center gap-2.5 bg-danger-light/10 border border-danger/20 text-danger-dark dark:text-danger rounded-xl p-3 text-xs mb-6"
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Responsively structured grid layout */}
            ...
            <div className="mt-1 space-y-1.5 bg-surface-100/50 dark:bg-white/[0.02] p-3.5 rounded-2xl border border-surface-200/50 dark:border-white/[0.05]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1">
                <div className={`flex items-center gap-2 text-xs transition-colors ${checks.length ? 'text-success dark:text-success' : 'text-surface-400 dark:text-surface-500'}`}>
                  <span className="text-[14px] leading-none">{checks.length ? '✓' : '○'}</span>
                  <span>Minimum 8 characters</span>
                </div>
                <div className={`flex items-center gap-2 text-xs transition-colors ${checks.uppercase ? 'text-success dark:text-success' : 'text-surface-400 dark:text-surface-500'}`}>
                  <span className="text-[14px] leading-none">{checks.uppercase ? '✓' : '○'}</span>
                  <span>At least 1 uppercase letter</span>
                </div>
                <div className={`flex items-center gap-2 text-xs transition-colors ${checks.lowercase ? 'text-success dark:text-success' : 'text-surface-400 dark:text-surface-500'}`}>
                  <span className="text-[14px] leading-none">{checks.lowercase ? '✓' : '○'}</span>
                  <span>At least 1 lowercase letter</span>
                </div>
                <div className={`flex items-center gap-2 text-xs transition-colors ${checks.number ? 'text-success dark:text-success' : 'text-surface-400 dark:text-surface-500'}`}>
                  <span className="text-[14px] leading-none">{checks.number ? '✓' : '○'}</span>
